...
def fncAgregarErroresDataframedtf(dtf, lstValidacion, lstFilasError):
    ...
    for i in lstValidacion:
        fncAgregarColumnaValidaciondtf(
            dtf, i[1], fncAgregarErrorFilastr(dtf, lstFilasError))
...
def fncAgregarAnchoColumna(excWriter, bolNumeroHojas, dtf, strNombreHoja):
    if bolNumeroHojas == True:
        for i in dtf:
            ...
            excWriter.sheets[strNombreHoja].set_column(
                strColumna, strColumna, intAnchoColumna)
    else:
        for i in dtf:
...
def fncAgregarFormatoColumnasError(excWriter, lstValidar, strHojaExcel, lstNombresColumnas):
    ...
    for i in lstValidar:
        if i[2] == 'Registro ya existe':
            plantilla.write(i[1] + 1, lstNombresColumnas.index(i[0]),
                            str(i[3]) + ' Error: ' + i[2], e_type)
...
def fncRetornarListaDataFrame(lstConsultas, lstSerializadores, lstClaves, lstNuevasClaves):
    ...
    for i in range(0, len(lstConsultas)):
        ...
        lstDataframes.append(fncRetornarDataframe(
            qrsConsulta, jsnSerializador, strClave, strClaveNueva))
...
def fncRetornarDataGraficolst(strSet, strIndicator):
    lstDataGrafico = []
    lstCategorias = []
    lstIndicador = []
    # if intSubset == None:
    qrsIndicador = clsIndicadoresComercialesMdl.objects.filter(
            indicator= strIndicator, subset= strSet)
    # else:
    #     qrsIndicador = clsIndicadoresComercialesMdl.objects.filter(
    #         indicator=strIndicator, subset=intSubset)
    if qrsIndicador:
        dctVentaReal = {}
        dctObjetivoVenta = {}
        lstDataVentaReal = []
...
def fncRetornarDataSelectdct(strNombreTabla, StrColumnaDescripcion=None):
    ...
    if strNombreTabla == 'modulo_configuracion_clssalidasalmacenmdl':
        ...
        lstConsultaCatalogoClientes = fncConsultalst(
            strConsultaCatalogoClientes, [])
        ...
        dtfCatalogoClientes = pd.DataFrame(
            lstConsultaCatalogoClientes, columns=['id', 'city'])
        dtfCiudades = pd.DataFrame(
            lstConsultaCiudades, columns=['city', 'city_name'])
        dtfUnidos = dtfConsultaSalidas.merge(
            dtfCatalogoClientes, how='left', on='id')
