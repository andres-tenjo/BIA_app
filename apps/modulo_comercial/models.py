...
''' Tablas promociones'''
class clsPromocionesMdl(BaseModel):
    promotion_name = models.CharField('Promoción', max_length=200)
...
class clsDetallePedidosMdl(BaseModel):
    order = models.ForeignKey(clsPedidosMdl, on_delete=models.CASCADE, verbose_name='Pedido')
    ...
    def save(self, force_insert=False, force_update=False, using=None, 
        ...
        if user is not None:
            if not self.pk:
                ...
                self.user_update = user
        super(clsDetallePedidosMdl, self).save()
...
class clsVentasPerdidasMdl(BaseModel):
